...
shop = Coinchanger.new

describe 'coinchanger' do
  it 'returns 5 x 200 coins if given 1000' do
    expect(shop.change_number_of_times(1000, 200)).to eq [200, 200, 200, 200, 200]
  end

  [200, 100, 50, 20, 10, 5, 2, 1].each do |change|
    it "returns change when given change" do
      expect(shop.coinchanger(change)).to eq change
    end
  end

  it 'returns [200, 100] when given 300' do
    expect(shop.coinchanger(300)).to eq [200, 100]
  end

  it 'returns [200, 50] when given 250' do
    expect(shop.coinchanger(250)).to eq [200, 50]
  end

  # it 'returns [200, 100, 50] when given 350' do
  #   expect(coinchanger(350)).to eq [200, 100, 50]
  # end
